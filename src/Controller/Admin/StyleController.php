...
class StyleController extends AbstractController
{
     ...
     */
    public function listeStylesCompletePaginee(StyleRepository $repo, PaginatorInterface $paginator, Request $request): Response
    {
     ...
     */
    public function ajoutStyle(Style $style=null,Request $request, EntityManagerInterface $manager): Response
    {
        if ($style==null) {
            $style=new Style;
            $mode="ajouté";
        ...
        if($form->isSubmitted() && $form->isValid())
        {
            ...
            $manager->flush();
            $this->addFlash("success","Le style à bien été $mode");
    ...
    public function suppressionStyle(Style $style,EntityManagerInterface $manager): Response
    {
        ...
        if($nbAlbums>0){
            $this->addFlash("danger","Impossible de supprimer le style car il possède $nbAlbums album(s) associés");
        }else{
